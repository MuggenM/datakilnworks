...
| **Open Data Sharing (Delta Sharing protocol)** | ✅ Delta Sharing (open protocol, Databricks-to-any) | ⚠️ Secure Data Sharing (Snowflake accounts; reader accounts) | ✅ **Delta Sharing server**: shares and recipients with expiring, rotatable, revocable tokens, signed short-lived file links, time travel, per-recipient activity; **tables with masking or row filter policies are refused** (raw files cannot be governed); works with the standard `delta-sharing` clients | ⚠️ **Databricks** (change data feed, Delta format, S3-backed tables, per-recipient IP lists, OIDC federation and marketplace are not implemented here) |
