...
| **Table / Schema-Level Grants** | ✅ `GRANT SELECT` on tables, schemas, catalogs | ✅ `GRANT` on tables, schemas, databases | ✅ **Select / Modify on a table or schema for users and groups, additive to catalog ACLs; fail-closed SQL verification** | ⚠️ **Databricks / Snowflake** (no SQL `GRANT`/`REVOKE` statements, no column-level or future-grant syntax; the default `warehouse` catalog stays open to all users) |
